...
export default function page() {
  ...
  return (
    ...
    <div dir='rtl' >
     ...
     <h2 className={`text-primary text-center border-b border-second inline  text-3xl lg:text-6xl mt-5 lg:my-5 lg:mt-16 pb-5  ${new1.className}`}>
      {
        prayers.title
      }
     </h2>
     ...
     <div key={item.name} className='p-3 shadow-xl mx-4 lg:mx-20 rounded-pixel   my-4 bg-primary2 text-second  lg:pr-10 text-center border border-primary group hover:border-second  transition-all'>
      ...
      <p className={ `text-base lg:text-3xl lg:mr-10 text-start ${new1.className} group-hover:text-second `}>
      {item.name}
      </p>
      </div>
    {
      item.item.map((item)=>{
        ...
        <p className={`text-base lg:text-2xl lg:mr-10 pt-2 text-start ${new1.className} group-hover:text-second transition-all`}>{item.start} :</p>
        <h3 className={`text-lg lg:text-3xl my-2 font-medium   ${new1.className}  `}>
     {
      item.prayer
     }
      </h3>
      <p className={`text-base lg:text-xl lg:mr-10 ${new1.className} group-hover:text-second transition-all `}>
      {
      item.reference
     }
      </p> 
